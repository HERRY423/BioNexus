...
def evaluate_population_frequencies(
    subpop_frequencies: Dict[str, float],
    inheritance_mode: str = "autosomal_dominant",
    disease_prevalence: float = 1e-4
) -> Dict[str, Any]:
    """
    ...
    if popmax_af >= 0.05:
        ...
        rationale_list.append(f"PopMax allele frequency in {popmax_subpop.upper()} is {popmax_af*100:.2f}% (>= 5.0% BA1 threshold).")
    ...
    if popmax_af >= bs1_cutoff and "BA1" not in activated_rules:
        ...
        rationale_list.append(f"PopMax allele frequency in {popmax_subpop.upper()} is {popmax_af*100:.2f}% (>= {bs1_cutoff*100:.1f}% BS1 threshold for {inheritance_mode}).")
    ...
    if popmax_af < 0.00001 and not activated_rules:
        ...
        rationale_list.append(f"PopMax allele frequency ({popmax_af:.2e}) is below 0.001% PM2 rarity threshold across all populations.")
...
def main():
    ...
    parser.add_argument("--subpops", "-s", nargs="+", help="Key=value subpopulation frequencies (e.g. nfe=0.0002 afr=0.00001)")
    parser.add_argument("--mode", "-m", default="autosomal_dominant", choices=["autosomal_dominant", "autosomal_recessive", "x_linked"])
