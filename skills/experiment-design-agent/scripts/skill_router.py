...
SKILL_REGISTRY = {
    ...
    "spatial-transcriptomics": {
        ...
        "keywords": ["spatial", "visium", "xenium", "merfish", "niche", "deconvolution", "moran", "svg", "tissue slice"],
    ...
    "variant-interpretation": {
        ...
        "keywords": ["variant", "mutation", "acmg", "pathogenic", "clinvar", "gnomad", "splice", "pgx", "pharmacogenomics"],
        "primary_script": "skills/variant-interpretation/scripts/acmg_classifier.py"
    },
    "protein-structure-analysis": {
        "domain": "3D Protein Structure & Drug Design",
        "keywords": ["structure", "pdb", "alphafold", "plddt", "binding pocket", "docking", "vina", "tm-score", "druggability"],
...
def extract_biological_entities(query_text: str) -> Dict[str, List[str]]:
    ...
    var_matches = re.findall(r"\b(?:c\.[0-9+\-_]+[A-Za-z0-9>_]+|p\.[A-Za-z]+[0-9]+[A-Za-z*]+|[A-Z][0-9]+[A-Z]|chr[0-9XYMT]+:[0-9]+:[A-Z]+:[A-Z]+)\b", text, re.IGNORECASE)

    # Diseases / indications
    disease_keywords = ["cancer", "adenocarcinoma", "melanoma", "carcinoma", "lymphoma", "leukemia", "diabetes", "alzheimer", "cardiovascular"]
    diseases = [d for d in disease_keywords if d in text.lower()]

    return {
        "genes": genes,
        "variants": var_matches,
        "diseases": diseases
    }
...
def route_scientific_query(query_text: str) -> Dict[str, Any]:
    ...
    if primary_skill == "variant-interpretation" or entities["variants"]:
        ...
        execution_steps.append({"step": 2, "script": "variant-interpretation/scripts/acmg_classifier.py", "purpose": "ACMG/AMP 28-rule Bayesian classification"})
        execution_steps.append({"step": 3, "script": "variant-interpretation/scripts/pharmacogenomics.py", "purpose": "CPIC actionable PGx lookup"})
    elif primary_skill == "protein-structure-analysis" or ("structure" in text_lower or "docking" in text_lower):
        execution_steps.append({"step": 1, "tool": "search_alphafold", "purpose": "Retrieve AI predicted 3D structure & pLDDT"})
        execution_steps.append({"step": 2, "script": "protein-structure-analysis/scripts/binding_pocket_detector.py", "purpose": "Detect 3D cavity & druggability"})
        execution_steps.append({"step": 3, "script": "protein-structure-analysis/scripts/drugability_scorer.py", "purpose": "Target tractability assessment"})
    elif primary_skill == "spatial-transcriptomics":
        execution_steps.append({"step": 1, "script": "spatial-transcriptomics/scripts/spatial_preprocessing.py", "purpose": "Coordinate normalization & QC"})
        execution_steps.append({"step": 2, "script": "spatial-transcriptomics/scripts/spatial_clustering.py", "purpose": "Spatial-aware MRF domain clustering"})
        execution_steps.append({"step": 3, "script": "spatial-transcriptomics/scripts/spatial_niche_analysis.py", "purpose": "Microenvironment niche discovery"})
    else:
        execution_steps.append({"step": 1, "tool": "search_opentargets", "purpose": "Evaluate genetic target-disease associations"})
        execution_steps.append({"step": 2, "tool": "search_chembl", "purpose": "Query known bioactive small molecules"})
        execution_steps.append({"step": 3, "script": "knowledge-graph-augmentation/scripts/hypothesis_validator.py", "purpose": "GraphRAG topological validation"})
