...
- **No evidence, no pass**: each check is `已检查` / `发现问题` / `缺少证据`. Missing DE results, sample design, or a parseable `rank_genes_groups` object is `NEEDS_DATA` / `NOT_ASSESSED`, never `ROBUST_PASS`. Methods past-tense claims are taken only from execution records; recommended analysis is listed separately.
