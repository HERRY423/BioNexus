...
>
> This skill demonstrates how skills can support your data engineering tasks鈥攁utomating schema transformations, parsing instrument outputs, and generating production-ready code.
>
...
- **Raw data** 鈫?`measurement-document` (direct instrument readings)
- **Calculated data** 鈫?`calculated-data-aggregate-document` (derived values)
...
Unknown techniques, units, or sample roles generate **warnings** (not errors) to allow for forward compatibility. If Allotrope adds new values after December 2024, the validator won't block them鈥攊t will flag them for manual verification. Use `--strict` mode to treat warnings as errors if you need stricter validation.
...
鈹溾攢鈹€ SKILL.md                          # This file
鈹溾攢鈹€ scripts/
鈹?  鈹溾攢鈹€ convert_to_asm.py            # Main conversion script
鈹?  鈹溾攢鈹€ flatten_asm.py               # ASM 鈫?2D CSV conversion
鈹?  鈹溾攢鈹€ export_parser.py             # Generate standalone parser code
鈹?  鈹斺攢鈹€ validate_asm.py              # Validate ASM output quality
鈹斺攢鈹€ references/
    鈹溾攢鈹€ supported_instruments.md     # Full instrument list with Vendor enums
    鈹溾攢鈹€ asm_schema_overview.md       # ASM structure reference
    鈹溾攢鈹€ field_classification_guide.md # Where to put different field types
    鈹斺攢鈹€ flattening_guide.md          # How flattening works
```
...
```bash
pip install allotropy  # install into current environment (virtual env recommended)
```
