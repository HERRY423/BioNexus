...
def calculate_qc_metrics_fast(
    ...
    if sp.issparse(X):
        ...
        if len(mt_indices) > 0:
            ...
            pct_counts_mt = np.divide(total_counts_mt, total_counts, out=np.zeros_like(total_counts, dtype=float), where=total_counts > 0) * 100
        else:
        ...
        if len(ribo_indices) > 0:
            ...
            pct_counts_ribo = np.divide(total_counts_ribo, total_counts, out=np.zeros_like(total_counts, dtype=float), where=total_counts > 0) * 100
        else:
        ...
        if len(hb_indices) > 0:
            ...
            pct_counts_hb = np.divide(total_counts_hb, total_counts, out=np.zeros_like(total_counts, dtype=float), where=total_counts > 0) * 100
        else:
...
def calculate_qc_metrics_chunked(
    ...
    for start in range(0, n_cells, chunk_size):
        ...
        if len(mt_idx) > 0:
            ...
            obs_metrics['pct_counts_mt'][start:end] = np.divide(mt_counts, sub_counts, out=np.zeros_like(sub_counts, dtype=float), where=sub_counts > 0) * 100

        # ribo
        ribo_idx = np.where(adata.var['ribo'].values)[0]
        if len(ribo_idx) > 0:
            ribo_counts = np.asarray(sub_X[:, ribo_idx].sum(axis=1)).ravel()
            obs_metrics['pct_counts_ribo'][start:end] = np.divide(ribo_counts, sub_counts, out=np.zeros_like(sub_counts, dtype=float), where=sub_counts > 0) * 100
