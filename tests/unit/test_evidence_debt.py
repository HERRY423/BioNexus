...
def test_epistemic_keystone_payoff_multiplier():
    ...
    # Top priority in repayment schedule should have high payoff multiplier / priority score
    top_pri = report.optimal_repayment_schedule[0]
    assert top_pri.payoff_multiplier >= 40.0
    assert top_pri.priority_score >= 40.0
...
def test_render_debt_reports():
    ...
    assert "HEURISTIC REMEDIATION PRIORITY SCHEDULE" in term
    assert "PRIORITY NOTE" in term

    md = render_markdown_debt_report(report)
    assert "# 📑 BioNexus Scientific Evidence Debt Report" in md
    assert "Heuristic Remediation Priority Schedule" in md
    assert "Transparency Note" in md
...
def test_cli_debt_payoff(tmp_path, capsys):
    ...
    captured = capsys.readouterr()
    assert "Heuristic Remediation Priority Schedule" in captured.out
