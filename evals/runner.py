...
def run_benchmark(
    ...
    Cases are split into two tracks (BNS-LC-004..006):
    - Gating track: guaranteed behavior; drives CRI and CI.
    - Frontier track (known_limitation: true): honest probes beyond current
      guarantees; reported with real pass/fail but excluded from gating metrics
      until graduation.
    """
    ...
    union_accuracy = (union_passed / union_total) if union_total > 0 else 0.0
...
def format_benchmark_markdown(report: BenchmarkReport) -> str:
    ...
    lines.append(
        f"**Timestamp**: `{report.timestamp}` | **Gating Cases**: `{report.total_cases}` | **Gating Accuracy**: `{report.overall_accuracy * 100:.1f}%`"
    )
