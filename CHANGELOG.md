...
### 🧪 Added (Lab-Grade Deployment — digests, lockfile, SBOM, offline mode, profiles, executed scale evidence)

- **Reproducible image (`container/apptainer.def`)**: base image pinned by OCI digest (`python:3.11-slim-bookworm` @ `sha256:0bee7276...`); CI re-resolves the tag from Docker Hub and fails the build on drift. The Python stack installs from `container/requirements-lock.txt` (208 packages, exact versions + wheel SHA-256 hashes via `uv pip compile --python-version 3.11 --generate-hashes`; pip hash-checking mode verifies wheels at install time, doubling as the offline supply-chain control) plus `--no-deps -e .` for the source tree. Image `%test` now also runs the offline deployment gate.
- **SBOM (`container/sbom-python.json`)**: CycloneDX 1.5 with the exact locked versions (209 components) via `scripts/generate_sbom.py --from-lockfile`; CI regenerates it inside the built image and asserts component-level equality with the committed file (SBOM drift fails the build).
- **Deployment manifest (`container/DEPLOYMENT_MANIFEST.json`)**: single ledger for base-image digest, lockfile/SBOM SHA-256, CPU/GPU/offline profiles, Slurm reference profiles, and scale-benchmark evidence with machine-class labels; digest consistency is test-enforced.
- **Offline mode (`BIONEXUS_OFFLINE=1`)**: deployment-level switch that forces the `OFFLINE_STRICT` egress mode (zero hosted endpoints) regardless of other configuration and cannot be relaxed at runtime; new `bionexus offline-check [--enforce]` gate and `bionexus doctor --offline/--require-offline` report + fail-closed gate. Verified without network access; runs in the image `%test` and the Slurm reference profiles.
- **CPU/GPU profiles**: one SIF, two run profiles — CPU (`apptainer exec`) CI-validated; GPU (`apptainer exec --nv` + node CUDA assertion) documented and exercised on HPC nodes (CI has no GPU, stated honestly).
- **Slurm reference profiles (`cluster/slurm/profiles/`)**: `hpc-cpu.sbatch` (doctor -> offline gate -> three-gate chain), `hpc-gpu.sbatch` (`--nv` + node-level CUDA evidence), `run_scale_benchmark.sbatch` (HPC reproduction of the committed benchmark evidence). Live-scheduler submission remains site adaptation, as before.
- **Scale benchmark: really executed at 500k and 1M cells (`evals/scale_benchmark.py` rework)**: memory-bounded by construction (chunked zero-inflated sparse generation at a controlled structural density, in-place CPM/log1p, streaming HVG) — the previous harness's 500k-cell "cluster reference" (~78%-dense Poisson draw) was not actually runnable anywhere. Windows peak-memory reporting added (psutil `peak_wset` / GetProcessMemoryInfo). Committed executed reports with machine fingerprints: 30k (validation, 16.6 s, 0.87 GB), **500k cells @ 8% density (278 s, 3.75 GB peak)**, **1M cells @ 5% density (482 s, 4.13 GB peak)** on an 8-core / 8 GB node — honestly labeled single small-node runs, NOT HPC numbers; top-lab HPC evidence is the sbatch reproduction committing its node's report.
- **Tests**: `tests/unit/test_lab_grade_deployment.py` (18 tests) — offline enforcement/refusal, gate exit codes, harness density + memory method, manifest/lockfile/SBOM digest and component consistency, container digest pin, Slurm profile content.
