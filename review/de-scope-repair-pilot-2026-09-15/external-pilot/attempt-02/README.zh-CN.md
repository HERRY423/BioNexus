...
`attempt-02/PILOT_STATUS.json` 是机器可读的诚实状态。当前真实外部任务、外部站点、合格双审阅任务
和完整成本记录均为 0；因此先导尚未执行。此目录只表示启动材料和本地校验已准备。

## 外部保管者操作顺序

1. 阅读 `PROTOCOL.zh-CN.md`，确认任务、站点、审阅者均符合独立性条件。
2. 为每个真实任务填写 `attempt-02/TASK_INTAKE_TEMPLATE.json`，计算数据集和任务包 SHA-256。
3. 把任务写入 `attempt-02/PLAN_TEMPLATE.json`，设置冻结时间；在外部受控位置保存计划摘要。
4. 用生产命令生成观察表：
