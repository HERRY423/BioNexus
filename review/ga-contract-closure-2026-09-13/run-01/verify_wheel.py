...
TARGET = OUTPUT / "wheel-install/site-packages"
sys.path.insert(0, str(TARGET))
temporary = OUTPUT / "tmp/wheel"
...
code = pytest.main([str(test_root), "-q", "-p", "no:cacheprovider",
                    "--basetemp", str(OUTPUT / "tmp/wheel-pytest"),
                    "--junitxml=" + str(OUTPUT / "wheel-tests.xml")])
assert Path(module.__file__).is_relative_to(TARGET)
with (OUTPUT / "wheel-result.json").open("x", encoding="utf-8") as handle:
