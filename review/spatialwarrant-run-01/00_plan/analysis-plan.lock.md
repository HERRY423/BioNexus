# SpatialWarrant — analysis-plan.lock.md

Plan version: SW-R01-S0-v1
Plan lifecycle: PREREGISTERED_AWAITING_HUMAN_APPROVAL
Approval / scientific results / analysis resource usage / machine verdict / Human Scientific Adjudication: **PENDING**
Inspection date: 2026-09-05 UTC (2026-09-04 America/Los_Angeles)
Run root: C:\Plugin\BioNexus\review\spatialwarrant-run-01
Current permission: S0 read-only checks and creation of this preregistration package only.
...
New choices resolved by this preregistration: primary distance 2 spot spacings, explicit boundary/core disjointness, exact gene-set release target, primary score and pairing algorithm, and resource/parameter gates below. They are proposals awaiting the user's hash-bound approval; they were chosen without opening expression matrices or viewing project results.
...
Required future tree (only 00_plan is created now):

    spatialwarrant-run-01/
      00_plan/
        analysis-plan.lock.md
        plugin-capability-check.json
        plan.sha256
      01_inputs/
      02_identity/
      03_scrna_reference/
...
Capacity policy (not a forecast): before any S1 data download or new analysis process, require at least 10 GiB free on the destination volume AND a documented per-stage storage plan covering acquired archives, extraction, retained copies and outputs. Maintain the 10 GiB floor while writing; stop before exhaustion and retain resumable partial files. Current C free space is below the floor. No automatic cleanup, movement to another drive, installation, or paid/remote compute is authorized. Define a measured runtime memory limit and checkpoint policy before long execution; do not start if safe allocation cannot be bounded. Actual required/peak RAM and duration stay PENDING.
...
Global/current stop:
- S0 package complete → stop. No approval has yet been supplied.
...
Open items at S0:
1. User hash-bound approval and later named scientific adjudicator.
2. Storage floor and per-stage capacity plan; no cleanup authorized.
...
Copy the single sentence supplied with the final plan SHA-256 in the assistant's final response. It authorizes this complete design while limiting the next work to S1. The user should switch the same task to Sol as specified in the original guide; the plan itself does not switch models.

Approval of exploratory computation does not accept biological conclusions, authorize bypass of storage/dependency/identity gates, authorize later stage transitions without checkpoints, or authorize publication. After S1, stop and present S1-checkpoint.md; do not automatically proceed to S2.
