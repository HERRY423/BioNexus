...
def tangram_worker(args: tuple[str, list[str]]) -> dict[str, Any]:
    ...
    for requested in TRAIN_GENE_COUNTS:
        ...
        for seed in SEEDS:
            ...
            record = {"section": sid, "requested_training_genes": requested, "actual_training_genes": len(genes),
            ...
            try:
                ...
                record.update({"status": "COMPLETED", "ended_at_utc": now(), "elapsed_seconds": time.perf_counter()-t0,
...
def section_coords(sid: str) -> pd.DataFrame:
    return pd.read_csv(S3/sid/"spot-QC-and-metadata.csv.gz", index_col="barcode")
