...
4. **Historical Report Provenance Quarantine**: The legacy report synchronizer (`sync_flagship_reports.py`) has been retired to read-only mode. Reports whose metadata was historically synced without fresh pipeline execution are quarantined under `validation/history/pre-ga-provenance-01/`.
...
2. **Create a Fresh Directory**: Execute the audit or verification into a new output directory:
   ```bash
   bionexus audit-de --de-table path/to/results.csv --sample-sheet path/to/samples.csv --claim "GENE is significant." --bundle path/to/new_review_output/
   ```
